...
exports.deleteUserData = functions
    ...
    await (async () => {
        ...
        const railwayStripeAnonymize = () => new Promise((resolve) => {
        ...
        try {
            ...
            if (resp.ok) {
                ...
                functions.logger.info(`[deleteUserData] ${key}: ok — anonymized=${anonymized} reason=${reason !== null && reason !== void 0 ? reason : "n/a"}`);
                steps[key] = { status: "ok", count: anonymized ? 1 : 0 };
            }
            else {
                // Fail-soft: record as error but do not abort.
                functions.logger.error(`[deleteUserData] ${key}: Railway error — ${resp.error} — continuing with Firestore deletion`);
    ...
    await (async () => {
        ...
        try {
            ...
            if (policy.authored_content === "delete") {
                ...
                const count = await paginatedAnonymise(query, { creatorId: null, creatorName: null }, dryRun);
                steps[key] = { status: "ok", count };
            }
            functions.logger.info(`[deleteUserData] ${key}: processed ${steps[key].count} docs (dryRun=${dryRun})`);
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : String(err);
            steps[key] = { status: "error", count: 0, error: msg };
...
exports.verifyBusiness = functions
    ...
    const uid = context.auth.uid;
    const method = data === null || data === void 0 ? void 0 : data.method;
    // ── Companies House path ─────────────────────────────────────────────────
    if (method === "companies_house") {
        const rawNum = String((data === null || data === void 0 ? void 0 : data.companyNumber) || "");
        ...
        if (!chResp.ok) {
            throw new functions.https.HttpsError("unavailable", "Verification service unavailable.");
        }
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const company = await chResp.json();
        if (company.company_status !== "active") {
            throw new functions.https.HttpsError("failed-precondition", "Company is not active.");
        }
        // verifiedBusinessName is ALWAYS from the API response — never client input.
        const verifiedName = company.company_name;
        ...
        functions.logger.info(`[verifyBusiness] companies_house verified uid=${uid} company=${companyNumber} name="${verifiedName}"`);
        return { verified: true, businessName: verifiedName };
    }
    // ── HMRC VAT path ────────────────────────────────────────────────────────
    if (method === "hmrc_vat") {
        const rawVat = String((data === null || data === void 0 ? void 0 : data.vatNumber) || "");
        ...
        if (!/^\d{9}(\d{3})?$/.test(vatNumber)) {
            throw new functions.https.HttpsError("invalid-argument", "Invalid VAT number format.");
        }
        ...
        functions.logger.info(`[verifyBusiness] hmrc_vat verified uid=${uid} vat=${vatNumber} name="${verifiedName}"`);
        return { verified: true, businessName: verifiedName };
    }
    ...
    if (method === "sole_trader_declaration") {
        const legalName = String((data === null || data === void 0 ? void 0 : data.legalName) || "").trim();
        const tradingName = String((data === null || data === void 0 ? void 0 : data.tradingName) || "").trim();
        const rawUtr = String((data === null || data === void 0 ? void 0 : data.utrNumber) || "");
        const utrNumber = rawUtr.replace(/\s/g, "").trim();
        if (!legalName || !tradingName) {
            throw new functions.https.HttpsError("invalid-argument", "Legal and trading name are required.");
        }
        // UK UTR is 10 digits. Validate format only — we cannot verify it is real.
        if (!/^\d{10}$/.test(utrNumber)) {
            throw new functions.https.HttpsError("invalid-argument", "Invalid UTR format (expected 10 digits).");
        }
        ...
        functions.logger.info(`[verifyBusiness] sole_trader_declaration recorded uid=${uid} trading="${tradingName}"`);
        return { verified: false, selfDeclared: true, businessName: tradingName };
    }
    // ── Unknown method ───────────────────────────────────────────────────────
    throw new functions.https.HttpsError("invalid-argument", "Unknown verification method.");
});
// ═══════════════════════════════════════════════════════════════════════════
...
function _normaliseDmText(text) {
    return text
        .toLowerCase()
        .replace(/[*@!0$]/g, "");
}
...
async function _vertexClassifyText(text) {
    ...
    const CLASSIFY_VERTEX_URL = "https://aiplatform.googleapis.com/v1/projects/huddl-connect" +
...
exports.moderateAndSendDM = functions
    ...
    const conversationId = String((_a = data.conversationId) !== null && _a !== void 0 ? _a : "").trim();
    if (!conversationId) {
    ...
    const convData = (_b = convSnap.data()) !== null && _b !== void 0 ? _b : {};
    ...
    const meetupData = (data.meetupData != null && typeof data.meetupData === "object") ? data.meetupData : null;
    ...
    if (groupData != null) {
        ...
        displayText = `\u{1F4C4} ${documentName !== null && documentName !== void 0 ? documentName : "Document"}`;
    }
    ...
    await db.collection("conversations").doc(conversationId).set(summaryUpdate, { merge: true });
    functions.logger.info(`[moderateAndSendDM] sent conversationId=${conversationId} messageId=${messageId} uid=${uid} type=${type}`);
    ...
    if (compositeText.length > 0 && geminiVerdict === null) {
        try {
            ...
            functions.logger.error(`[moderateAndSendDM] flag doc write failed: ${String(flagErr)}`);
        }
    ...
    return new Promise((resolve) => {
        ...
        const req = https.request(options, (res) => {
            ...
            res.on("end", () => {
                clearTimeout(timeout);
                if (res.statusCode && res.statusCode >= 400) {
                    functions.logger.warn(`[onDmMessageCreated] Railway notify-dm returned HTTP ${res.statusCode}`);
                }
                resolve();
            });
        });
...
exports.onDmMessageCreated = functions
    ...
    try {
        ...
        // ── 1. Sender ──────────────────────────────────────────────────────────
        const senderId = String((_a = msg.senderId) !== null && _a !== void 0 ? _a : "").trim();
        if (!senderId) {
            functions.logger.warn(`[onDmMessageCreated] messageId=${messageId} has no senderId — skipping`);
            return;
        }
        // ── 2. Participants ────────────────────────────────────────────────────
        const convSnap = await db.collection("conversations").doc(conversationId).get();
        if (!convSnap.exists) {
            functions.logger.warn(`[onDmMessageCreated] conversation ${conversationId} not found — skipping`);
            return;
        }
        const participants = Array.isArray((_b = convSnap.data()) === null || _b === void 0 ? void 0 : _b["participants"])
            ? convSnap.data()["participants"]
            : [];
        if (participants.length === 0) {
            functions.logger.warn(`[onDmMessageCreated] conversation ${conversationId} has no participants — skipping`);
            return;
        }
        // ── 3. Recipient ───────────────────────────────────────────────────────
        ...
        if (blk.exists)
            return; // recipient blocked sender — skip push
        ...
        const type = String((_d = msg.type) !== null && _d !== void 0 ? _d : "text");
        const rawMessage = String((_e = msg.message) !== null && _e !== void 0 ? _e : "");
        ...
        let messagePreview;
        if (groupData != null) {
            messagePreview = `\u{1F465} Group: ${String((_f = groupData["name"]) !== null && _f !== void 0 ? _f : "Group")}`;
        }
        else if (itemData != null) {
            messagePreview = `\u{1F4E6} Item: ${String((_g = itemData["title"]) !== null && _g !== void 0 ? _g : "Item")}`;
        }
        else if (meetupData != null) {
            messagePreview = `\u{1F4C5} Meetup: ${String((_h = meetupData["title"]) !== null && _h !== void 0 ? _h : "Meetup")}`;
        }
        else if (eventData != null) {
            messagePreview = `\u{1F4C5} Event: ${String((_j = eventData["title"]) !== null && _j !== void 0 ? _j : "Event")}`;
        }
        else if (type === "image") {
            ...
            messagePreview = `\u{1F4C4} ${documentName !== null && documentName !== void 0 ? documentName : "Document"}`;
        }
        else if (type === "location") {
        ...
        const secret = process.env.INTERNAL_SERVICE_SECRET;
        if (!secret) {
            functions.logger.warn("[onDmMessageCreated] INTERNAL_SERVICE_SECRET not set — notification skipped");
            return;
        }
        ...
        functions.logger.info(`[onDmMessageCreated] notified recipientId=${recipientId} for messageId=${messageId} type=${type}`);
    }
    catch (err) {
        // Never throw from a Firestore trigger — retries could spam notifications.
        functions.logger.error(`[onDmMessageCreated] unhandled error for messageId=${context.params.messageId}: ${String(err)}`);
    }
});
// ═══════════════════════════════════════════════════════════════════════════════
...
// ═══════════════════════════════════════════════════════════════════════════════
exports.moderateAndSendGroupMessage = functions
    .region("europe-west2")
    .runWith({
    timeoutSeconds: 30,
    memory: "256MB",
    ...
    const groupId = String((_a = data.groupId) !== null && _a !== void 0 ? _a : "").trim();
    if (!groupId) {
        throw new functions.https.HttpsError("invalid-argument", "groupId is required.");
    }
    const groupSnap = await db.collection("groups").doc(groupId).get();
    if (!groupSnap.exists) {
        throw new functions.https.HttpsError("permission-denied", "Group not found.");
    }
    ...
    const pollOptions = (Array.isArray(data.pollOptions)) ? data.pollOptions : null;
    ...
    const msgData = Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign(Object.assign({ groupId, senderId: uid, // FORCED from auth — never from data
        senderName, // resolved from users/{uid} server-side
        senderAvatar, message: messageText, timestamp: admin.firestore.FieldValue.serverTimestamp(), reactions: {}, pinned: false, isSystem: false, isMeetupCard: false, attachments: [], replyToText,
        replyToSender, meetupData: null, type,
        audioUrl,
        audioDuration, liveExpired: false, clientTempId }, (imageUrl != null && { imageUrl })), (latitude != null && { latitude })), (longitude != null && { longitude })), (locationLabel != null && { locationLabel })), (liveUntil != null && { liveUntil })), (contactName != null && { contactName })), (contactPhone != null && { contactPhone })), (documentUrl != null && { documentUrl })), (documentName != null && { documentName })), (documentSize != null && { documentSize })), (documentMimeType != null && { documentMimeType })), (pollQuestion != null && { pollQuestion })), (pollOptions != null && { pollOptions })), (pollAllowMultiple != null && { pollAllowMultiple })), (pollExpiresAt != null && { pollExpiresAt })), (pollIsCalendarMode != null && { pollIsCalendarMode })), (pollFirestoreId != null && { pollFirestoreId }));
    ...
    if (grpCompositeText.length > 0 && geminiVerdict === null) {
        try {
            ...
            // Non-fatal: message is already written. Log and continue.
            functions.logger.error(`[moderateAndSendGroupMessage] flag doc write failed: ${String(flagErr)}`);
        }
        return { status: "flagged", messageId };
    }
    return { status: "sent", messageId };
...
// }
// ═══════════════════════════════════════════════════════════════════════════
exports.seedWelcomeSubscription = functions
    .region("europe-west2") // REGION-RESIDENCY-1
    .firestore.document("users/{userId}")
    .onCreate(async (_snap, context) => {
    var _a, _b;
    const userId = context.params.userId;
    functions.logger.info(`[seedWelcomeSubscription] Triggered for userId=${userId}`);
    ...
    if (existing.exists) {
        functions.logger.info(`[seedWelcomeSubscription] subscriptions/${userId} already exists ` +
            `(tier=${(_b = (_a = existing.data()) === null || _a === void 0 ? void 0 : _a.tier) !== null && _b !== void 0 ? _b : "unknown"}) — skipping seed.`);
        return;
    }
    // ── SEED WELCOME DOC ──────────────────────────────────────────────────
    ...
    try {
        ...
        functions.logger.info(`[seedWelcomeSubscription] Successfully seeded welcome subscription for userId=${userId}`);
    }
    catch (err) {
        // Log and re-throw so Cloud Functions marks this invocation as failed
        // and Cloud Tasks / the retry policy will attempt again.
        functions.logger.error(`[seedWelcomeSubscription] Failed to seed subscription for userId=${userId}: ${String(err)}`);
        throw err;
    }
});
 ...
 */
function _outwardCode(postcode) {
 ...
 */
async function _resolveBorough(uid, payloadBorough, postcode) {
    ...
    if (postcode && postcode.trim() !== "") {
        const outward = _outwardCode(postcode);
        const networkBorough = await new Promise((resolve) => {
            const options = {
                hostname: "api.postcodes.io",
            ...
            const req = https.request(options, (res) => {
                ...
                res.on("end", () => {
                    ...
                    try {
                        const parsed = JSON.parse(raw);
                        const districts = (_a = parsed.result) === null || _a === void 0 ? void 0 : _a.admin_district;
        ...
        if (networkBorough !== "")
            return networkBorough;
    }
    return "";
}
exports.joinDefaultGroups = functions
    .region("europe-west2")
    .runWith({ timeoutSeconds: 60, memory: "256MB" })
    .https.onCall(async (data, context) => {
    var _a, _b, _c, _d;
    // ── Auth guard ───────────────────────────────────────────────────────────
    if (!context.auth) {
        throw new functions.https.HttpsError("unauthenticated", "Must be signed in to join groups.");
    }
    const uid = context.auth.uid;
    // ── Input validation ─────────────────────────────────────────────────────
    const input = data;
    if (!Array.isArray(input.groups) || input.groups.length === 0) {
        throw new functions.https.HttpsError("invalid-argument", "groups must be a non-empty array.");
    }
    ...
    for (const spec of groups) {
        ...
        try {
            ...
            const resolvedBorough = await _resolveBorough(uid, (_a = spec.borough) !== null && _a !== void 0 ? _a : "", (_b = spec.postcode) !== null && _b !== void 0 ? _b : "");
            // Hard-fail if borough cannot be resolved at all — surface in failed[]
            // so it shows up in logs and the caller's return value.
            if (resolvedBorough === "") {
                const reason = `borough_unresolved:${(_c = spec.postcode) !== null && _c !== void 0 ? _c : ""}`;
                functions.logger.error(`[joinDefaultGroups] BOROUGH_UNRESOLVED uid=${uid} groupId=${groupId} postcode=${(_d = spec.postcode) !== null && _d !== void 0 ? _d : ""}`);
            ...
            if (spec.borough && spec.borough.trim() !== "") {
                ...
                if (!boroughMatch) {
                    ...
                    functions.logger.warn(`[joinDefaultGroups] BLOCKED uid=${uid} groupId=${groupId} — ${reason}`);
                    failed.push({ id: groupId, reason });
                    continue;
                }
            }
            ...
            await db.runTransaction(async (tx) => {
                ...
                if (snap.exists) {
                    ...
                    const memberIds = (_a = existing["memberIds"]) !== null && _a !== void 0 ? _a : [];
                    if (memberIds.includes(uid)) {
                    ...
                    tx.update(ref, Object.assign(Object.assign({ memberIds: admin.firestore.FieldValue.arrayUnion(uid), memberCount: admin.firestore.FieldValue.increment(1), isImageLocked: true, groupType: "resident" }, (spec.level ? { level: spec.level } : {})), { updatedAt: admin.firestore.FieldValue.serverTimestamp() }));
                    outcome = "joined";
                }
                else {
                    ...
                    const docData = {
                        ...
                        postcode: (_b = spec.postcode) !== null && _b !== void 0 ? _b : "",
                        ...
                        groupType: "resident",
                        invitedMemberIds: [],
                        lastMessage: (_c = spec.lastMessage) !== null && _c !== void 0 ? _c : "Welcome to the group!",
                        lastSenderName: (_d = spec.lastSenderName) !== null && _d !== void 0 ? _d : "Huddl",
                        level: (_e = spec.level) !== null && _e !== void 0 ? _e : "borough",
                        lastMessageTime: admin.firestore.FieldValue.serverTimestamp(),
                        createdAt: admin.firestore.FieldValue.serverTimestamp(),
                        updatedAt: admin.firestore.FieldValue.serverTimestamp(),
                    };
                    if (spec.birthYear != null)
                        docData["birthYear"] = spec.birthYear;
                    if (spec.parentGroupId)
                        docData["parentGroupId"] = spec.parentGroupId;
                    tx.set(ref, docData);
                    outcome = "created";
                }
            });
            if (outcome === "created") {
                ...
                functions.logger.info(`[joinDefaultGroups] CREATED group=${groupId} name="${spec.name}" uid=${uid} borough=${spec.borough}`);
            }
            else if (outcome === "joined") {
                joined.push(groupId);
                functions.logger.info(`[joinDefaultGroups] JOINED group=${groupId} uid=${uid}`);
            }
            else {
                // already_member — counts as joined for the caller's purposes
                joined.push(groupId);
                functions.logger.info(`[joinDefaultGroups] ALREADY_MEMBER group=${groupId} uid=${uid} — no-op`);
            }
        }
        catch (err) {
            const reason = String(err);
            functions.logger.error(`[joinDefaultGroups] FAILED group=${groupId} uid=${uid} error=${reason}`);
            failed.push({ id: groupId, reason });
        }
    }
    functions.logger.info(`[joinDefaultGroups] DONE uid=${uid} ` +
