...
class _TripsScreenState extends State<TripsScreen> {
        ...
        children: [
          Expanded(
            child: Column(
              ...
              children: [
                Text('Trips', style: GoogleFonts.poppins(fontSize: 22, fontWeight: FontWeight.w700, color: HuddlColors.textDark)),
                Text('Plan, pack & ask parents who\'ve been there', style: GoogleFonts.poppins(fontSize: 12, color: HuddlColors.textSecondary)),
              ],
            ),
          ),
          _headerIcon(Icons.auto_awesome, HuddlColors.aiGradient, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AiCopilotScreen()))),
          const SizedBox(width: 10),
          _headerIcon(Icons.luggage_outlined, HuddlColors.primaryGradient, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const MyTripsScreen()))),
        ],
      ),
    );
  }

  Widget _headerIcon(IconData icon, LinearGradient gradient, VoidCallback onTap) {
    return GestureDetector(
      onTap: onTap,
      child: Container(
        width: 38, height: 38,
        decoration: BoxDecoration(gradient: gradient, borderRadius: BorderRadius.circular(12)),
        child: Icon(icon, color: HuddlColors.white, size: 18),
      ),
    );
  }
  ...
  Widget _quickActions() {
    return Padding(
      ...
      child: Row(
        children: [
          _action(Icons.chat_bubble_outline_rounded, 'Ask Parents', HuddlColors.primary, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AskParentsScreen()))),
          const SizedBox(width: 10),
          _action(Icons.lightbulb_outline_rounded, 'Tips', HuddlColors.teal, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CommunityTipsScreen()))),
          const SizedBox(width: 10),
          _action(Icons.auto_awesome_outlined, 'AI Concierge', HuddlColors.blue, () => Navigator.push(context, MaterialPageRoute(builder: (_) => const TravelConciergeScreen()))),
        ],
      ),
    );
  }
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Text(label, style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: HuddlColors.textDark)),
            ],
          ),
        ),
      ),
  ...
  // ── Needs help — unanswered questions ───────────────────────────────────
  Widget _needsHelp() {
    ...
    return _section(
      ...
      action: 'See all',
      onAction: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AskParentsScreen())),
      child: Column(
        children: unanswered.map((q) => _questionCard(q, urgent: true)).toList(),
      ),
    );
  }
  ...
  Widget _activeConversations() {
    ...
    return _section(
      ...
      onAction: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AskParentsScreen())),
      child: Column(
        children: active.map((q) => _questionCard(q, urgent: false)).toList(),
      ),
    );
  }
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          color: HuddlColors.white,
          borderRadius: BorderRadius.circular(14),
          border: urgent ? Border.all(color: HuddlColors.primary.withValues(alpha: 0.25)) : null,
            ...
            Row(
              children: [
                CircleAvatar(
                  ...
                  child: Text(q.authorName[0], style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: color)),
                ),
                const SizedBox(width: 10),
                Expanded(
                ...
                if (urgent)
                  ...
                  Container(
                    ...
                    child: Text('$replies ${replies == 1 ? 'reply' : 'replies'}', style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w500, color: HuddlColors.teal)),
                  ),
              ],
            ),
            const SizedBox(height: 10),
            Text(q.question, style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w500, color: HuddlColors.textDark, height: 1.4), maxLines: 2, overflow: TextOverflow.ellipsis),
            // Tags
            ...
            if (last != null) ...[
              ...
              Container(
                ...
                child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  CircleAvatar(
                    radius: 12,
                    backgroundColor: _hex(last.authorAvatarColor).withValues(alpha: 0.15),
      ...
      child: Column(
        ...
        children: [
          Padding(
            ...
            child: _sectionHeader('Top tips from parents', action: 'See all', onAction: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CommunityTipsScreen()))),
          ),
          const SizedBox(height: 10),
          SizedBox(
            height: 140,
            child: ListView.builder(
              ...
              itemBuilder: (_, i) {
                ...
                return GestureDetector(
                  ...
                  child: Container(
                    width: 250,
                    margin: EdgeInsets.only(right: i < tips.length - 1 ? 10 : 0),
                    padding: const EdgeInsets.all(14),
                    decoration: BoxDecoration(
                      color: HuddlColors.white,
                      borderRadius: BorderRadius.circular(14),
                      ...
                      children: [
                        Row(children: [
                          CircleAvatar(
                            ...
                            child: Text(tip.authorName[0], style: GoogleFonts.poppins(fontSize: 10, fontWeight: FontWeight.w600, color: _hex(tip.authorAvatarColor))),
                          ),
                          const SizedBox(width: 8),
                          Expanded(child: Text(tip.authorName, style: GoogleFonts.poppins(fontSize: 12, fontWeight: FontWeight.w600, color: HuddlColors.textDark))),
                          _chip(tip.destination, HuddlColors.blue),
                        ]),
                        const SizedBox(height: 8),
                        Expanded(
                          child: Text(tip.tip, style: GoogleFonts.poppins(fontSize: 12, color: HuddlColors.textSecondary, height: 1.4), maxLines: 3, overflow: TextOverflow.ellipsis),
                        ),
                        Row(children: [
                          const Icon(Icons.thumb_up_alt_outlined, size: 13, color: HuddlColors.textHint),
                          const SizedBox(width: 4),
                          Text('${tip.upvotes}', style: GoogleFonts.poppins(fontSize: 11, color: HuddlColors.textHint)),
                          const SizedBox(width: 10),
  ...
  Widget _destinations() {
    ...
    final emojis = {'Tenerife': '🌴', 'Mallorca': '🏖️', 'Algarve': '🌊', 'Costa del Sol': '☀️', 'Lake Garda': '⛵', 'Cornwall': '🏄', 'Cotswolds': '🌿', 'Crete': '🏛️'};
    final colors = {'Tenerife': const Color(0xFFFF975C), 'Mallorca': const Color(0xFF3580F0), 'Algarve': const Color(0xFF199A85), 'Costa del Sol': const Color(0xFFF3C54F), 'Lake Garda': const Color(0xFF5B9DFF), 'Cornwall': const Color(0xFF22C55E), 'Cotswolds': const Color(0xFF78B0FF), 'Crete': const Color(0xFFF69F72)};
                    ...
                    child: Column(
                      ...
                      children: [
                          ...
                          child: Center(
                            child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                              ...
                              Container(
                                ...
                                child: Text('${d.huddlParentsVisited} families', style: GoogleFonts.poppins(fontSize: 9, fontWeight: FontWeight.w600, color: c)),
                              ),
                            ]),
                          ),
                        ),
                        Padding(
                          padding: const EdgeInsets.all(10),
                          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Text(d.name, style: GoogleFonts.poppins(fontSize: 13, fontWeight: FontWeight.w600, color: HuddlColors.textDark)),
                            Text('${d.country} · ${d.flightTime}', style: GoogleFonts.poppins(fontSize: 10, color: HuddlColors.textHint)),
                            const SizedBox(height: 3),
                            Row(children: [
                              const Icon(Icons.star, size: 12, color: HuddlColors.accentAmber),
                              const SizedBox(width: 2),
                              Text('${d.rating}', style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: HuddlColors.textDark)),
                              const SizedBox(width: 4),
                              Text('${d.recommendPercent}% rec', style: GoogleFonts.poppins(fontSize: 9, color: HuddlColors.teal, fontWeight: FontWeight.w500)),
                            ]),
                          ]),
                        ),
                      ],
        ...
        child: Container(
          ...
          child: Row(
            children: [
                  ...
                  children: experts.asMap().entries.map((e) {
                    ...
                    return Positioned(
                      ...
                      child: CircleAvatar(
                        ...
                        child: CircleAvatar(
                          radius: 16, backgroundColor: c.withValues(alpha: 0.15),
                          child: Text(e.value.name[0], style: GoogleFonts.poppins(fontSize: 11, fontWeight: FontWeight.w600, color: c)),
                        ),
                      ),
                    );
                  }).toList(),
              ...
              Expanded(
                child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Parent experts', style: GoogleFonts.poppins(fontSize: 14, fontWeight: FontWeight.w600, color: HuddlColors.textDark)),
                  Text('${experts.length} experienced parents sharing travel tips', style: GoogleFonts.poppins(fontSize: 11, color: HuddlColors.textSecondary)),
                ]),
              ),
              const Icon(Icons.arrow_forward_ios, size: 14, color: HuddlColors.textHint),
            ],
          ),
        ),
      ),
  ...
  Widget _sectionHeader(String title, {String? action, VoidCallback? onAction}) {
    return Row(
      children: [
        Text(title, style: GoogleFonts.poppins(fontSize: 16, fontWeight: FontWeight.w600, color: HuddlColors.textDark)),
        const Spacer(),
        if (action != null)
          GestureDetector(
            onTap: onAction,
