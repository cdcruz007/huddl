...
async function createCheckoutSession({ userId, email, productId, successUrl, cancelUrl }) {
  ...
  const sessionParams = {
    ...
    success_url:
      successUrl ||
      `${process.env.STRIPE_SUCCESS_URL || 'https://www.huddlapp.co.uk/subscription/success'}?session_id={CHECKOUT_SESSION_ID}`,
    cancel_url:
      cancelUrl ||
      `${process.env.STRIPE_CANCEL_URL || 'https://www.huddlapp.co.uk/subscription/cancel'}`,
    metadata: {
      userId,
      productId,
...
async function createPortalSession(userId) {
  ...
  const session = await stripe.billingPortal.sessions.create({
    ...
    return_url: process.env.FRONTEND_URL || 'https://www.huddlapp.co.uk',
  });
